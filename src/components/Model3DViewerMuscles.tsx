...
export function Model3DViewerMuscles({ title, onClose }: { title: string, onClose: () => void }) {
    ...
    <AnimatePresence>
      ...
      >
        <motion.div className="bg-white rounded-lg p-4 w-full max-w-3xl">
          ...
          <div className="w-full h-[500px] bg-red-200 rounded-lg flex items-center justify-center">
            <Canvas
              ref={canvasRef}
              camera={{ position: [0, 0, 20], fov: 50 }} // Ajusta la posición y el campo de visión (fov) de la cámara
            >
              <ambientLight intensity={0.5} />
              <directionalLight position={[0, 10, 5]} intensity={3} />
              <Suspense fallback={null}>
                <Model path="/modelos/Musculos.gltf" />
              </Suspense>
              <OrbitControls
                enableZoom={true}
